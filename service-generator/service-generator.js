...
const API_URI_PREFIX = '/api' //pentru capri web unde nu au /api in constanta, pt share = ''
const httpDependecy = {
    name: 'http',
    type: 'HttpClient',
    scope: 'private'
...
const defaultImports = [{
    ...
    moduleSpecifier: '../common/endpoint-constants/constants',
    ...
    resolveParamTypeBySchema(schema) {
        if(schema.type) {
            if(schema.type == 'integer') {
                ...
                return this.resolveParamTypeBySchema(schema.items) + '[]';
            }
            return schema.type;
        } else if(schema["$ref"]) {
